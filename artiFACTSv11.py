...
# --- Discogs API helpers (personal token via env: DISCOGS_TOKEN) ---
...
def discogs_search_release(artist: str = "", title: str = "", barcode: str = "", catno: str = "", per_page: int = 10):
    ...
    if catno:
        params["catno"] = catno
    try:
        r = requests.get("https://api.discogs.com/database/search", headers=headers, params=params, timeout=15)
        r.raise_for_status()
...
def discogs_get_release(release_id: int):
    ...
    try:
        r = requests.get(f"https://api.discogs.com/releases/{release_id}", headers=headers, params=params, timeout=15)
...
def _pick_best_discogs_match(results, artist: str = "", title: str = ""):
    if not results:
        return None
...
def map_discogs_release_to_schema(release: dict) -> dict:
    ...
    def pos(i,tr):
        p = (tr.get("position") or "").strip()
        return p if p else str(i+1)
    tracklist = "; ".join(f"{pos(i,tr)} {tr.get('title','').strip()}" for i,tr in enumerate(tracks) if tr.get("title"))
    ...
    out = {
        ...
        "contributors_primary": contributors_primary,
...
def merge_gpt_and_discogs(gpt: dict, discogs: dict) -> dict:
    ...
    prefer = ["artist","release_title","label","labels_all","country","release_year",
              "genres","styles","tracklist","format","size","rpm","weight_grams","color",
...
def enrich_vinyl(name: str, photo_path: str, helpers: dict) -> dict:
    """Hybrid vinyl enrichment: GPT (image + hints) -> Discogs -> merge."""
    client = _openai_client()
    ...
    guidance = (
        "- If the release is clearly a commercial album, attempt to fill label, year, country, and tracklist.\n"
        "- Prefer evidence visible in the image or text hints; otherwise leave blank."
    )
    ...
    try:
        ...
        gpt_parsed = json.loads(text[s_idx:e_idx+1]) if (s_idx != -1 and e_idx != -1 and e_idx > s_idx) else {}
    ...
    try:
        ...
        if "—" in name:
            ...
            if len(parts) == 2:
                artist_q, title_q = parts[0], parts[1]
        if not artist_q:
            artist_q = (gpt_parsed.get("artist") or "").strip()
        if not title_q:
            title_q  = (gpt_parsed.get("release_title") or "").strip()

        barcode_q = (gpt_parsed.get("barcode_ean_upc") or "").split(";")[0].strip() if isinstance(gpt_parsed.get("barcode_ean_upc"), str) else ""
        catno_q   = (gpt_parsed.get("catalog_number") or "").strip() if isinstance(gpt_parsed.get("catalog_number"), str) else ""

        results = discogs_search_release(artist=artist_q, title=title_q, barcode=barcode_q, catno=catno_q)
        best = _pick_best_discogs_match(results, artist=artist_q, title=title_q)
        if best and best.get("id"):
            release = discogs_get_release(best["id"])
            discogs_data = map_discogs_release_to_schema(release) if release else {}
    except Exception:
        discogs_data = {}
...
SCHEMAS = {
    ...
    "vinyl": {
        "user": ["name", "purchase_price", "storage_or_display_location", "notes",
                 "runout_deadwax_code_side_a", "runout_deadwax_code_side_b"],
        "api": [
            "artist", "release_title", "label", "labels_all", "country", "release_year", "genres", "styles", "tracklist", "total_runtime", "format", "size", "rpm", "weight_grams", "color", "pressing_plant", "edition_notes", "contributors_primary", "discogs_median_price_usd", "discogs_low_high_usd", "description", "fact"
        ]
    },
    "coin": {
        "user": ["name", "collected_date", "provenance", "purchase_price", "storage_or_display_location", "notes"],
